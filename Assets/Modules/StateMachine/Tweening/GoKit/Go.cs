...
namespace ViitorCloud.GameHelper.GoKit {
	public class Go : MonoBehaviour {
		...
		private static void log(object format, params object[] paramList) {
			if (format is string)
				Log(string.Format(format as string, paramList));
			else
				Log((string)format);
		}
		...
		public static void warn(object format, params object[] paramList) {
			...
			if (format is string)
				LogWarning(string.Format(format as string, paramList));
			else
				Log((string)format);
		}
		...
		public static void error(object format, params object[] paramList) {
			...
			if (format is string)
				LogError(string.Format(format as string, paramList));
			else
				Log((string)format);
		}
		...
		public static void addTween(AbstractGoTween tween) {
			...
			if (instance != null && !instance.enabled) // purposely using the static instace property just once for initialization
			...
			if (_instance != null && !_instance._timeScaleIndependentUpdateIsRunning && tween.updateType == GoUpdateType.TimeScaleIndependentUpdate)
