...
namespace ViitorCloud.GameHelper.GoKit {
	public class GoTween : AbstractGoTween {
		...
		public GoTween(object target, float duration, GoTweenConfig config, Action<AbstractGoTween> onComplete = null) {
			...
			this.targetName = target is UnityEngine.Component ? (target as UnityEngine.Component).name : "<no name>";
#endif
...
#if SHOW_NOISY_TWEEN_WARNING
					LogWarning("target validation failed. destroying the tween to avoid errors. Target type: " + this.targetTypeString + " Target name: " + this.targetName + " Duration: " + this.duration.ToString());
#endif
					autoRemoveOnComplete = true;
					return true;
				}
		...
		public void addTweenProperty(AbstractTweenProperty tweenProp) {
			...
			if (tweenProp.validateTarget(target)) {
				...
				if (_tweenPropertyList.Contains(tweenProp)) {
					Log("not adding tween property because one already exists: " + tweenProp);
					return;
				}
				...
				Log("tween failed to validate target: " + tweenProp);
			}
		}
