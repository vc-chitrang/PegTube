	...
	public static class GoTweenUtils {
		...
		public static Func<float, float, float, float, float> easeFunctionForType(GoEaseType easeType) {
			switch (easeType) {
				...
				case GoEaseType.CubicOutIn:
					return GoEaseCubic.EaseOutIn;
				...
				case GoEaseType.QuadOutIn:
					return GoEaseQuadratic.EaseOutIn;
		...
		public static T setterForProperty<T>(System.Object targetObject, string propertyName) {
			...
			if (propInfo == null) {
				LogError("could not find property with name: " + propertyName);
				return default(T);
			}
