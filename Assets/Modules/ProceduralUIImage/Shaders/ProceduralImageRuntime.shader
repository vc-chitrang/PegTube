Shader "UI/Procedural UI Image"
{
	Properties
	{
        ...
        _ReverseGradient ("ReverseGradient", int) = 0

        _TopColor ("TopColor", Color) = (1.0, 1.0, 1.0, 1.0)
        _BotColor ("BotColor", Color) = (1.0, 1.0, 1.0, 1.0)
	}
	SubShader
	{
		Tags
		...
		Pass
		{
			...
			v2f vert(appdata_t IN){
				...
				#endif
				OUT.color = IN.color*_Color*(1+_TextureSampleAdd);
				if(_ReverseGradient == 0)
					OUT.colorCoord = half2(1.0 - IN.texcoord.x, IN.texcoord.y);
				else
					OUT.colorCoord = half2(IN.texcoord.x, IN.texcoord.y);
				return OUT;
			}
			...
			// PERF ISSUE* old hardwares seem to have difficulty 
			// with this method. Re enabled old visible method
			...
			{
             	half4 color = IN.color * (0.5 * lerp(_BotColor, _TopColor, IN.colorCoord.y) + 0.5 * lerp(_BotColor, _TopColor, IN.colorCoord.x));
